...
def get_diff(original, edited, min_char_len=0, min_word_len = 0):
    
    sm = SequenceMatcher(a=original, b=edited)
    op_codes = sm.get_opcodes()

    valid_op = op_codes[0][0] == 'equal' 
    valid_op = valid_op and len(op_codes) > 1
    valid_op = valid_op and op_codes[1][0] == 'insert'
    valid_op = valid_op and (op_codes[1][4] -  op_codes[1][3]) > min_char_len
    
    i, j = op_codes[1][3], op_codes[1][4]
    edited_target = edited[i:j]
    
    valid_op = valid_op and len(edited_target.split()) > min_word_len
    ...
    common_prefix = original[:cp_i]
    original_target = original[i:]
    edited_target = edited[i:j]
    
    return {'prefix': common_prefix, 'original': original_target,'edited': edited_target}
...
def has_valid_url(url_list):
    return any([ any([d in url for url in urls]) for d in domains ])
...
if __name__ == "__main__":
    ...
    for line in tqdm(lines):
        ...
        if True:
            ...
            original = line['original_src_text']
            edited = line['original_tgt_text']

            diff = get_diff(original, edited, min_char_len=100, min_word_len=10)
        
        

            urls = [] if not diff else url_regex.findall(diff['original']) + url_regex.findall(diff['edited'])
            if not has_valid_url(urls):
                diff = None
    ...
    data_out = data # [d for d in data_ if valid_data(d)]
